...
function SubstreamWrapper(mux, id) {
  ...
  this._mux.on('error', this.errorListener);
...
SubstreamWrapper.prototype.stopErrorListening = function() {
  this.debug('stop error listening');
  this._mux.removeListener('error', this.errorListener);
};
...
MuxWrapper.prototype.removeStream = function(id) {
  ...
  if (this.substreams[id]) {
    this.substreams[id].stopErrorListening();
