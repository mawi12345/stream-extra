...
function UDPWrapper(socket, options) {
  ...
  this._boundProcessMessage = this._processMessage.bind(this);
  this._errorHandler = function(err) {
    _this.emit('error', err);
  };

  this._socket.on('message', this._boundProcessMessage);
...
UDPWrapper.prototype.close = function() {
  debug('wrapper close');
  this._socket.removeListener('message', this._boundProcessMessage);
...
UDPWrapper.prototype._getResendTimeout = function(count) {
  var timeout = count * this._resendTimeout;
  if (timeout <= 0) {
    timeout = this._resendTimeout;
  } else if (timeout > this._maxResendTimeout) {
    timeout = this._maxResendTimeou;
  }
...
UDPWrapper.prototype._sendMessage = function() {
  if (this._stopped || !this._messages.length) {
    return null;
  }
  ...
  if (!msg.waitingForACK) {
    ...
    msg.resendTimeout = setTimeout(function() {
      ...
      msg.needResend = true;
      _this._sendMessage();
    }, this._getResendTimeout(msg.count));
...
UDPWrapper.createServer = function(socket, options, connectionListener) {
  ...
  var register = {};
  var holePunchingRegister = {};
  var onMessage = function(data, rinfo) {
    var id = rinfo.address + ':' + rinfo.port;
    if (!register[id]) {
      if (holePunchingRegister[id]) {
        debug('stop hole punching ' + id);
        clearTimeout(holePunchingRegister[id]);
        holePunchingRegister[id] = false;
      }
  ...
  return {
    close: function() {
      ...
      Object.keys(register).forEach(function(id) {
        var stream = register[id];
        stream.close();
      });
    },

    addHolePunchingTarget: function(host, port, interval) {
      var id = host + ':' + port;
      debug('add hole punching target ' + id);
      if (register[id]) {
        debug('register conatins hole punching target ' + id + ' ignoreing request');
        return;
      }

      if (holePunchingRegister[id]) {
        debug('hole punching register conatins target ' + id + ' ignoreing request');
        return;
      }

      holePunchingRegister[id] = setInterval(function() {
        debug('hole punching ' + id);
        var punch = new Buffer(1);
        punch[0] = 255;
        this._socket.send(punch, 0, punch.length, port, host);
      }, interval || 2000);
    },
  };
};
